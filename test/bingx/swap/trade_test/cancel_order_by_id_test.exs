defmodule BingX.Swap.TradeTest.CancelOrderByIDTest do
  @moduledoc """
  ...
  alias BingX.Swap.{CancelOrderResponse, Trade, Order}
  ...
  describe "BingX.Swap.Trade cancel_order_by_id/4" do
    test "should make DELETE request", context do
      ...
      Trade.cancel_order_by_id("BTC-USDT", "id", api_key, secret_key)
    ...
    test "should put the symbol and order ID into query params", context do
      ...
      Trade.cancel_order_by_id(symbol, order_id, api_key, secret_key)
    ...
    test "should return the original request http-error", context do
      ...
      assert ^error = Trade.cancel_order_by_id("BTC-USDT", "ORDER-ID", api_key, secret_key)
    end
    ...
    test "should extract and validate response content", context do
      ...
      Trade.cancel_order_by_id("BTC-USDT", "ORDER-ID", api_key, secret_key)
    ...
    test "should wrap the content into CancelOrderResponse struct", context do
      ...
      assert {:ok, ^struct} = Trade.cancel_order_by_id("BTC-USDT", "ORDER-ID", api_key, secret_key)
