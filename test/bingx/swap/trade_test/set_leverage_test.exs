defmodule BingX.Swap.TradeTest.SetLeverageTest do
  @moduledoc """
  ...
  alias BingX.Swap.{Trade, SetLeverageResponse}
  alias BingX.HTTP.{Client, Response, Error}

  setup_all do
    {
      :ok,
      api_key: "API_KEY_FOR_TEST", secret_key: "SECRET_KEY_FOR_TEST", path: "/openApi/swap/v2/trade/leverage"
    }
  end

  describe "BingX.Swap.Trade set_leverage/5" do
    test "should make POST request", context do
      ...
      Trade.set_leverage("BTC-USDT", :long, 100, api_key, secret_key)
    ...
    test "should put the symbol, position_side and leverage into query params", context do
      %{api_key: api_key, secret_key: secret_key} = context
    ...
    test "should return the original request http-error", context do
      ...
      assert ^error = Trade.set_leverage("BTC-USDT", :long, 100, api_key, secret_key)
    end
    ...
    test "should extract and validate response content", context do
      ...
      patch(SetLeverageResponse, :new, struct)
    ...
    test "should wrap the content into SetLeverageResponse struct", context do
      %{api_key: api_key, secret_key: secret_key} = context
      ...
      assert {:ok, ^struct} = Trade.set_leverage("BTC-USDT", :long, 100, api_key, secret_key)

      assert_called_once(SetLeverageResponse.new(^content))
      assert_called_once(Response.process_response(^response))
    end
