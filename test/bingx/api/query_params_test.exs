defmodule BingX.API.QueryParamsTest do
  use ExUnit.Case
  use Patch

  alias BingX.API.QueryParams
  ...
  describe "BingX.API.QueryParams append_signature/2" do
    ...
    test "should convert the provided params into query string to make signature", context do
      %{secret_key: secret_key} = context

      patch(QueryParams, :signature, & &1)
    ...
    test "should append correct signature", context do
      ...
      patch(QueryParams, :signature, signature)

      assert %{"signature" => ^signature} = QueryParams.append_signature(params, secret_key)
    end
  end

  describe "BingX.API.QueryParams signature/2" do
    test "should encrypt raw correctly using the provided secret key", _context do
      secret_key = "#__$#@_$_)(#@)"

      raw = "VALUE_TO_ENCRYPT"

      exp_result =
        Crypto.mac(:hmac, :sha256, secret_key, raw)
        |> Base.encode16(case: :lower)

      assert ^exp_result = QueryParams.signature(raw, secret_key)
    end
  end
end
